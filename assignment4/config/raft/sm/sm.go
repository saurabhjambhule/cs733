...
func (appReq AppEntrReq) send(sm *State_Machine) {
	...
	case FOLL:
		...
		if sm.CurrTerm < appReq.Term {
			fmt.Println(sm.CurrTerm, "}}Follower -", appReq.LeaderId, ":", sm.Id, "#", sm.LoggInd, "....", appReq)
			...
			prevInd := sm.LoggInd - int32(len(appReq.Logg.Logg)) //+ int32(1)
			respLog := LoggStore{Data: (sm.Logg.Logg[prevInd:])}

			fmt.Println(sm.Id, ")", prevInd, "-", sm.LoggInd, "...", sm.Logg, "...", respLog)
		...
		//}
		prevInd := sm.LoggInd - int32(len(appReq.Logg.Logg)) + int32(1)
		fmt.Println(sm.Id, ")", prevInd, "-", sm.LoggInd, "...", sm.Logg)
...
func (appRes AppEntrResp) send(sm *State_Machine) {
	...
	case LEAD:
		...
		if appRes.Succ == false {
			...
			//respp := LoggStore{Data: sm.Logg.Logg[sm.LoggInd-1]}
...
func (app Append) commit(sm *State_Machine) {
	...
	case LEAD:
		fmt.Println(sm.CurrTerm, "} Leader -", sm.Id, "....", string(app.Data))
		...
		resp := LoggStore{Data: []MyLogg{{sm.CurrTerm, string(app.Data)}}}
		...
		if len(sm.Logg.Logg) == 0 {
			...
			respp = Send{PeerId: 0, Event: AppEntrReq{Term: sm.CurrTerm, LeaderId: sm.Id, PreLoggInd: sm.LoggInd - 1, PreLoggTerm: sm.Logg.Logg[sm.LoggInd-1].Term, LeaderCom: sm.CommitIndex, Logg: entry}}
		}
		sm.CommMedium.ActionCh <- resp
		sm.CommMedium.ActionCh <- respp
		sm.CommMedium.CommitCh <- CommitInfo{Data: []byte(sm.Logg.Logg[sm.LoggInd-1].Logg), Err: nil, Index: sm.LoggInd - 1}
		resppp := Alarm{T: FTO}
		sm.CommMedium.ActionCh <- resppp
	}
}
